...
const resumoRepetidos = document.getElementById('resumoRepetidos');
...
function processarArquivo(texto) {
  ...
  dados = [];
  let somaQtdRegistroRepetido = 0;
  let totalNsrsComRepeticao = 0;
  ...
  nsrMap.forEach((identMap, nsr) => {
    ...
    if (total > 1) {
      ...
      somaQtdRegistroRepetido += total;
      totalNsrsComRepeticao++;
    }
  });

  if (totalNsrsComRepeticao === 0) {
    resumoRepetidos.textContent = '✅ Registro Íntegro – Nenhum NSR repetido encontrado.';
    resumoRepetidos.style.color = '#00c853';
  } else {
    resumoRepetidos.innerHTML = `🔁 <strong>Total de NSR repetidos:</strong> ${somaQtdRegistroRepetido} &nbsp;&nbsp;&nbsp; 🔁 <strong>Total de NSR afetado:</strong> ${dados.reduce((acc, d) => acc + d.detalhes.length, 0)}`;
    resumoRepetidos.style.color = '#ffab00';
  }
